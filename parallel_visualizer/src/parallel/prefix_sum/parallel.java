...
// javac -cp "$SPARK_HOME/jars/*" parallel.java
// java -cp "$SPARK_HOME/jars/*:." parallel



public class parallel {

    public static long[] prefixSum(int[] input, JavaSparkContext sc) {
        int n = input.length;
    ...
    public static void main(String[] args) {
        try {
            ...
            SparkConf conf = new SparkConf().setAppName("Spark Prefix Sum").setMaster("local");
            JavaSparkContext sc = new JavaSparkContext(conf);

            long start = System.nanoTime();
            long[] result = prefixSum(v, sc);
